...
// Select review for a specific product


// public function getReviewsByProductCode($prdt_id) {
//     $stmt = $this->db->prepare('SELECT * FROM reviews WHERE prdt_id = :prdt_id');
//     $stmt->execute([':prdt_id' => $prdt_id]);
//     return $stmt->fetchAll(PDO::FETCH_ASSOC);
// }

// select all reviews
public function getAllReviews() {
    $allReviews = $this->db->prepare("SELECT * FROM reviews");
    $allReviews->execute();
    return $allReviews->fetchAll(PDO::FETCH_ASSOC);
}
